...
export default async function getOldStatsAlbums(
  ...
  const albums = await transaction
    ...
    .where('nextStatsUpdateAt', '<=', sql<Date>`NOW()`)
    .orderBy('nextStatsUpdateAt', 'asc')
    .limit(limit)
    .execute();
  ...
  await transaction
    .updateTable('Album')
    .set({ nextStatsUpdateAt: sql<Date>`NOW() + interval '24 hours'` })
